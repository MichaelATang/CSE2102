 <?php 
   if (($_SESSION['logged_in_user_type'] == 'admin') || ($_SESSION['logged_in_user_type'] == 'officer')) {
	   ?>
<h1>Technicians <span class="GxNoPrint">| <a href="?technicians=yes&add=yes" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-plus"></span> Add New</a> | <a href="?technicians=yes&export=yes" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-plus"></span> Mailing List</a></span></h1>
<?php
//if export
...
//if add do
	...
	print "<meta http-equiv=\"refresh\" content=\"0;URL=?technicians=yes\">";
} else {
	//just list all users
	if (isset($_GET['show'])) {
		$results = getContactById($_GET['show']);
	} else $results = getAllTechnicianContacts(); 
?>
...
foreach($results as $row){
    ...
    <td>(w) <?php echo $row['office_phone']; ?><br />
        ...
        (m) <?php echo $row['mobile_phone']; ?><br />
			</td>
    <td class="GxNoPrint"><?php if (($_SESSION['logged_in_user_type'] == 'admin') || ($_SESSION['logged_in_user_type'] == 'officer')) { ?><a class="btn btn-xs btn-danger glyphicon glyphicon-trash" href="#" title="DELETE RECORD" onClick="linkRef('?technicians=yes&del=<?php echo $row['id']; ?>')"></a>  <a class="btn btn-xs btn-info glyphicon glyphicon-pencil" title="Edit Contact" href="?technicians=yes&edit=<?php echo $row['id']; ?>"></a>  <?php } ?><a class="btn btn-xs btn-success glyphicon glyphicon-zoom-in" title="View Contact" href="?technicians=yes&view=<?php echo $row['id']; ?>"></a></td>
  </tr>
<?php        
}
?>
